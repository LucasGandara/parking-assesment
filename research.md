...
Delegate to the @librarian agent:
...
@librarian Where are the authentication handlers in this codebase?

# Understanding patterns
@librarian How does this codebase handle error responses? Show me the pattern.

# Finding integration points
@librarian I need to add a new API endpoint for user preferences. Where should it go and what patterns should it follow?

# Understanding flows
@librarian Trace how a request flows from the API route to the database for user creation.

# Finding similar implementations
@librarian Find examples of form validation in this codebase so I can follow the same pattern.
```
...
```
# First query
@librarian How does authentication work in this codebase?

# Librarian returns high-level overview

# Follow-up query
@librarian Thanks. Now specifically, how are JWT tokens validated? I need to add a new claim check.
```
